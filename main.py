...
from src.bbc import BertBinaryClassifier
from src.utils import evaluate, read_data_1a, save_output_file, set_seed
...
def pipeline(hyper_params:list):
    ...
    result_path = f"output/1a_test/dev/task1A_output_{run_name}.tsv"
    result_path2 = f"output/1a_test/test/task1A_output_{run_name}.tsv"
...
if __name__ == "__main__":
    ...
    # exps_1a.yaml contains the hyperparameters for the experiments of the task 1a
    with open("exps_1a.yaml", 'r') as stream:
        data_loaded = yaml.safe_load(stream)

    
    exps =data_loaded['exps_final']
